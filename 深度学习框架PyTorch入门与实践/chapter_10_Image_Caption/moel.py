...
class CaptionModel(nn.Module):
    ...
    def forward(self, img_feats,captions,lengths):
        ...
        packed_embedding=pack_padded_sequence(embeddings,lengths)
        outputs,state=self.rnn(packed_embedding)
        pred=self.classifier(outputs[0])
        return pred,state

    def genearte(self,img,eos_token='</EOS>',
                 beam_size=3,max_caption_length=30,length_normalization_factor=0.0):
        ...
        return sentences
    def states(self):
        opt_state_dict={attr:getattr(self.opt,attr) for attr in dir(self.opt) if not attr.startswith('_')}
    ...
    def save(self,path=None,**kwargs):
        if path is None:
            path='{prefix}_{time}'.format(prefix=self.opt.prefix,time=time.strftime('%m%d_%H%M'))
        states=self.states()
